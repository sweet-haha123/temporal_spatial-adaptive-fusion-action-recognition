...
class BaseDataset(Dataset):
    ...
    def get_data_info(self, idx: int) -> dict:
        ...
        if self.serialize_data:
            ...
            data_info = copy.deepcopy(self.data_list[0][idx])  # edited
        ...
        ir_data_info = copy.deepcopy(self.data_list[1][idx])  # edited
        # Some codebase needs `sample_idx` of data information. Here we convert
        # the idx to a positive number and save it in data information.
        if idx >= 0:
            ir_data_info['sample_idx'] = idx
        else:
            ir_data_info['sample_idx'] = len(self) + idx

        return data_info, ir_data_info
    ...
    def full_init(self):
        ...
        # serialize data_list
        if self.serialize_data:
            self.data_bytes, self.data_address,self.ir_data_bytes, self.ir_data_address= self._serialize_data()
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        if self.test_mode:
            data1,data2= self.prepare_data(idx)
            data=[]
            data.append(data1)
            data.append(data2)
            if data is None:
    ...
    def _serialize_data(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        data_list = [_serialize(x) for x in self.data_list[0]]  # edited
        ...
        # added by myself
        ir_data_list = [_serialize(x) for x in self.data_list[1]]
        ir_address_list = np.asarray([len(x) for x in ir_data_list], dtype=np.int64)
        ir_data_address: np.ndarray = np.cumsum(ir_address_list)
        # TODO Check if np.concatenate is necessary
        ir_data_bytes = np.concatenate(ir_data_list)

        gc.collect()
        return data_bytes, data_address, ir_data_bytes, ir_data_address
    ...
    def prepare_data(self, idx) -> Any:
        ...
        """
        data_info, ir_data_info = self.get_data_info(idx)
        ir_data_info = self.pipeline(ir_data_info)
