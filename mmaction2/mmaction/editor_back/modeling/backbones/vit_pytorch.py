...
# from mmaction.editor_back.modeling.fusion_part.OCFR import OCFR
import matplotlib.pyplot as plt
# import seaborn as sns  # 使用Seaborn库绘制KDE图

from mmaction.editor_back.modeling.fusion_part.OCFR import  OCFR
# from plug_module.localvit import LocalityFeedForward
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.norm = build_norm_layer(dict(type='LN', eps=1e-6), 768)[1]
...
class BlockMask(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., num_class=12, qkv_bias=False, qk_scale=None, drop=0.,
                 attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, momentum=0.9):
        super().__init__()
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.normR =build_norm_layer(dict(type='LN', eps=1e-6), 768)[1]
        self.attnR = AttentionMask(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop,
                                   proj_drop=drop)
        self.normR_ =build_norm_layer(dict(type='LN', eps=1e-6), 768)[1]
        self.mlpR = MlpMasked(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop,
                              qkv_bias=qkv_bias)
        self.normN = build_norm_layer(dict(type='LN', eps=1e-6), 768)[1]
        self.attnN = AttentionMask(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop,
                                   proj_drop=drop)
        self.normN_ = build_norm_layer(dict(type='LN', eps=1e-6), 768)[1]
        self.mlpN = MlpMasked(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop,
                              qkv_bias=qkv_bias)
        self.attn_vis = AttentionMask(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop,
                                  proj_drop=drop)
        self.attn_ir = AttentionMask(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop,
                                  proj_drop=drop)
        self.attn = AttentionMask(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop,
                                   proj_drop=drop)
        # self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop,
        #                           proj_drop=drop)
        self.mlpR = MlpMasked(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop,
        ...
        # self.cross_attn=CrossAttentionMask(dim=dim)

        # self.conv = LocalityFeedForward(dim, dim, 1, mlp_ratio, 'hs+se', 4, False, False)
    ...
    def forward(self, x, mask):
        ...
        # x= x + self.drop_path(self.attn(self.norm(x)))
        # x = x + self.drop_path(self.attn(self.norm(x)))
        x= x + self.drop_path(self.attn(self.norm(x), mask=mask))
        x = x + self.drop_path(self.attn(self.norm(x), mask=mask))

        # x = x + self.drop_path(self.mlp(self.norm(x)))
        x = x + self.drop_path(self.mlp(self.norm(x), mask=mask))
...
class BlockFuse(nn.Module):
    ...
    def forward(self, x):
        ...
        # return x[:,0]
        # else:
        #     x = x + self.drop_path(self.attn1(self.norm(x), mask=mask))
        #     x = x + self.drop_path(self.mlp(self.norm(x), mask=mask))
        #     x = self.norm(x)
        #     mask2 = mask.repeat(2, 1, 1)
        #     x = x * mask2
        #     return x[:,0],None
