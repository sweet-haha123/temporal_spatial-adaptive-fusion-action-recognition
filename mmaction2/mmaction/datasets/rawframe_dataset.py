...
class RawframeDataset(BaseActionDataset):
    ...
    def load_data_list(self) -> List[dict]:
        ...
        for line in fin:
            ...
            exists(self.ir_file)
            ir_data_list = []
            fin = list_from_file(self.ir_file)
            for line in fin:
                line_split = line.strip().split()
                ir_video_info = {}
                idx = 0
                ...
                frame_dir = line_split[idx]
                if self.data_prefix['img'] is not None:
                    frame_dir = osp.join("/home/qingyuhan/data/ir_data/rawframes", frame_dir)
                ir_video_info['frame_dir'] = frame_dir
                idx += 1
                if self.with_offset:
                    # idx for offset and total_frames
                    ir_video_info['offset'] = int(line_split[idx])
                    ir_video_info['total_frames'] = int(line_split[idx + 1])
                    idx += 2
                else:
                    # idx for total_frames
                    ir_video_info['total_frames'] = int(line_split[idx])
                    idx += 1
                # idx for label[s]
                label = [int(x) for x in line_split[idx:]]
                # add fake label for inference datalist without label
                ...
                if self.multi_class:
                    assert self.num_classes is not None
                    ir_video_info['label'] = label
                else:
                    assert len(label) == 1
                    ir_video_info['label'] = label[0]
                ir_data_list.append(ir_video_info)

        return data_list,ir_data_list

    def get_data_info(self, idx: int) -> dict:
        """Get annotation by index."""
        data_info,ir_data_info = super().get_data_info(idx)
        data_info['filename_tmpl'] = self.filename_tmpl
        ir_data_info['filename_tmpl'] = self.filename_tmpl
