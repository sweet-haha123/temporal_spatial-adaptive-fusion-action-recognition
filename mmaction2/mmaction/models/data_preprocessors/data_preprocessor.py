...
from mmaction.editor_back.CrossNorm_SelfNorm import SelfNorm,CrossNorm
from plug_module.Crossnorm_Selfnorm import cn
...
class ActionDataPreprocessor(BaseDataPreprocessor):
    ...
    def preprocess(self,
        ...
        # selfnorm = SelfNorm(chan_num=3).cuda()
        # x1 = inputs[0].to(torch.float32).squeeze().permute(1, 0, 2, 3)
        # x2 = inputs[1].to(torch.float32).squeeze().permute(1, 0, 2, 3)
        # #
        # inputs[0] = selfnorm(x1).permute(1, 0, 2, 3).unsqueeze(0)
        # inputs[1] = selfnorm(x2).permute(1, 0, 2, 3).unsqueeze(0)
