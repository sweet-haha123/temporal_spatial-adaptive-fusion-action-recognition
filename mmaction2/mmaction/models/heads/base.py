...
# writer = TensorboardVisBackend(save_dir=)
from torch.utils.tensorboard import SummaryWriter
# writer=SummaryWriter(log_dir='/home/gaocq/code/mmaction2/true_false_v1')
log_epoch=0
...
class BaseHead(BaseModule, metaclass=ABCMeta):
    ...
    def loss(self, feats: Union[torch.Tensor, Tuple[torch.Tensor]],
        ...
        for i in range(len(feats)):
            cls_scores = self(feats[i], **kwargs)
            loss = self.loss_by_feat(cls_scores, data_samples)
            # loss=self.loss_by_feat(cls_scores.unsqueeze(0), data_samples)
            for key in loss.keys():
                if i==0:
                    loss_score[key] = loss[key]
                else:
                    if key=='loss_cls':
                        loss_score[key]+=loss[key]
        loss_score['loss_cls']/=len(feats)
        return loss_score
    ...
    def predict(self, feats: Union[torch.Tensor, Tuple[torch.Tensor]],
        ...
        #     cls_scores = self(feats, **kwargs)
        #     if i==0:
        #     self.predict_by_feat(cls_scores, data_samples)
        # cls_scores=0
        # for i in range(len(feats)):
        #     cls_scores=cls_scores+self(feats[i], **kwargs)
        # cls_scores=cls_scores/2
        cls_scores=self(feats, **kwargs)
    ...
    def predict_by_feat(self, cls_scores: torch.Tensor,
        ...
        # if pred_labels!=data_samples[0].gt_label:
        #     global log_epoch
        #     log_epoch += 1
        #     writer.add_text('Log','Ground--->>>Pred,{}--->>>{}'.format(data_samples[0].gt_label.item(),pred_labels.item()),log_epoch)
