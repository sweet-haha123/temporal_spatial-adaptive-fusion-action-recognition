...
class Recognizer3D(BaseRecognizer):
    ...
    def extract_feat(self,
        ...
        # # To convert datasamples to one-hot encoding
        # labels = [sample.gt_label.item() for sample in data_samples]
        # # 转换为 one-hot 编码
        # num_classes = 12
        # one_hot_labels = torch.zeros(len(labels), num_classes)
        #
        # # 平滑处理，使每个位置的概率分布变为类似于 softmax 形式
        # for idx in range(len(labels)):
        #     for i in range(num_classes):
        #         if i == labels[idx]:
        #             one_hot_labels[idx, i] = 1 - (num_classes - 1) * 0.01
        #         else:
        #             one_hot_labels[idx, i] = 0.01
        # gt_label = one_hot_labels.to('cuda')
        ...
        if test_mode:
            ...
            if self.test_cfg is not None and self.test_cfg.get(
                ...
                # from mmaction.models.backbones.videomae import TubeMaskingGenerator
                # loss_func = torch.nn.MSELoss()
                # self.mask = TubeMaskingGenerator((8, 14, 14), 0.75)
                # videos_squeeze = rearrange(inputs, 'b c (t p0) (h p1) (w p2) -> b (t h w) (p0 p1 p2) c', p0=2,
                #                            p1=16, p2=16)
                # videos_norm = (videos_squeeze - videos_squeeze.mean(dim=-2, keepdim=True)
                #                ) / (videos_squeeze.var(dim=-2, unbiased=True, keepdim=True).sqrt() + 1e-6)
                # # we find that the mean is about 0.48 and standard deviation is about 0.08.
                # videos_patch = rearrange(videos_norm, 'b n p c -> 1 (b n) (p c)')
                # bool_masked_pos = torch.tensor(self.mask() == 1).unsqueeze(0).cuda()
                # B, _, C = videos_patch.shape
                # labels = videos_patch[bool_masked_pos].reshape(B, -1, C)
                # x = self.backbone(inputs.view(1, 3, 16, 224, 224), bool_masked_pos)
                # loss_bcc = loss_func(x, labels)
                # loss_predict_kwargs['loss_bcc']=loss_bcc
                # end
