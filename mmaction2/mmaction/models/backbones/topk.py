...
class PatchNet(nn.Module):
    ...
    def forward(self, x, type, N, T, sigma,scores):
        B = x.size(0)
        H = W = int(sqrt(N))
        indicator = None
        indices = None
        ...
        if type == 'time':
            if self.score == 'tpool':
                ...
                if self.training:
                    keep_index=torch.argsort((torch.sum(indicator,dim=[0,2])),descending=True)[:12]
                else:
                    keep_index=indices.squeeze(0)
        else:
