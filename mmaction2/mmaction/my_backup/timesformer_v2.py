...
class TimeSformer(nn.Module):
    ...
    def __init__(self,
        ...
        self.temporal_select=PatchNet(score='tpool',k=12,in_channels=768)
        self.spatial_select = PatchNet(score='spatch', k=50, in_channels=768)
    ...
    def forward(self, x):
        ...
        temporal_scores=mean_attn_weights.sum(dim=0,keepdim=True)
        temporal_mask = torch.argsort(temporal_scores, descending=True).squeeze(0)
        # 对重要性得分进行排序
        x=rearrange(x[:,1:,:],'1 (t h w) c -> 1 c t h w',t=16,h=14,w=14)
        # x_tem,temporal_mask=self.temporal_select(x,type='time',N=196,T=16,sigma=0.05,scores=temporal_scores)


        x_tem=x[:,:,temporal_mask[:12],:,:]
        spatial_scores = last_map[:, 0, 1:][temporal_mask[:12]].unsqueeze(2)
        spatial_scores=rearrange(spatial_scores,'(b t) (h w) c ->(b t) c h w ',b=1,h=14,w=14)
        # spatial_scores = self.min_max_norm(spatial_scores)
        scores = F.unfold(spatial_scores, kernel_size=(5,9), stride=2)
        scores = scores.mean(dim=1)
        x_tem=rearrange(x_tem,'b c t h w -> (b t) c h w')
        x_spatial = F.unfold(x_tem, kernel_size=(5,9), stride=2).permute(0, 2, 1).contiguous()
        # x_spatial,_=self.spatial_select(x_tem,type='spatch',N=196,T=12,sigma=0.05,scores=spatial_scores)
        _,spatial_select_tmp=torch.topk(scores, k=1, dim=-1, sorted=False)

        spatial_select = spatial_select_tmp.view(-1, 1, 1).expand(-1, 1, x_spatial.size(2))
        feat=torch.gather(x_spatial,1,spatial_select)

        feat=feat.squeeze(1)
        feat=rearrange(feat,'(b t) (c kh kw) -> (b t) (kh kw) c',t=12,kh=5,kw=9)

        from mmengine.visualization import TensorboardVisBackend
        writer = TensorboardVisBackend(save_dir='/home/gaocq/code/mmaction2/vis_batch_v20')
        visualize_attention_with_mask_bbox(writer, rearrange(identity_x,'b c t h w ->(b t) c h w '), temporal_mask,spatial_select_tmp,
                            'ValVisible_Attention_Map')
        # feat, mask = self.SFTS(Vis_feat=x,
        #                         Vis_attn=last_map,
        #                         mask_fre=temporal_mask,
        #                         identity_x=identity_x)

        # x_spatial=rearrange(x_spatial,'b c t h w -> (b t) (h w) c')

        x=torch.cat((feature_global.repeat(12,1,1),feat),dim=1)
        for blk in self.mhsa_s:
            x, _ = blk(self.norm_s1(x))
        out_attn_s = self.mlp_s(self.norm_s2(x))
        space_global = rearrange(out_attn_s[:, 0], '(b t) d -> b t d', b=1, t=12, d=768)
...
def visualize_attention_with_mask_bbox(writer, images,  temporal_mask, patch_indices,tag, nrow=8):
    """
    Visualizes images with attention boxes based on a temporal mask in TensorBoard.

    :param writer: TensorBoard SummaryWriter object.
    :param images: Tensor of shape (num_images, 3, 224, 224), the input images.
    :param patch_indices: Tensor or list of patch indices (num_images,), one per image.
    :param temporal_mask: Tensor or list of booleans (num_images,), where True indicates the image should have a box.
    ...
    for j in range(num_images):
        ...
        # image=np.clip((image*127.5+127.5),0,255)
        ...
        if j in temporal_mask[:12]:  # If the mask is True, draw the attention box
            patch_idx = patch_indices[i].item()
            i+=1
            x = (patch_idx % ((14-5)//2)) * 14
            y = (patch_idx//((14-5)//2)) * 14
            w, h = 14 * 5, 14 * 9
            ...
            # combined_image=cv2.rectangle(image,(x,y),(x+w,y+h),(0, 0, 255), 2)
        else:
            # If the mask is False, just use the original image
            combined_image = image

        # Convert back to CHW format for TensorBoard
        combined_image = torch.tensor(combined_image)

        # Create an image grid for TensorBoard visualization
        img_grid = vutils.make_grid(combined_image, nrow=nrow, normalize=False, scale_each=False)
        writer.add_image(f'{tag}/image_{j}', img_grid)