...
#beta Beta分布的参数，用于控制剪裁区域大小的随机性
#设定的剪裁区域面积与整个图像面积的比率阈值
...
def cn(x, crop='neither', beta=1, bbx_thres=0.1, lam=None, chan=False):
    """2-instance crossnorm with cropping."""

    color=x[0].squeeze(0).permute(1,0,2,3)
    infraed=x[1].squeeze(0).permute(1,0,2,3)
    infraed_aug = instance_norm_mix(content_feat=infraed, style_feat=color)

    # if lam is not None:
    #     x = x * lam + x_aug * (1-lam)
    # else:
    #     x = x_aug
    out=torch.cat((color.permute(1,0,2,3).unsqueeze(0),infraed_aug.permute(1,0,2,3).unsqueeze(0)),dim=0).unsqueeze(1)
    return out
class CrossNorm(nn.Module):
    ...
    def __init__(self, crop=None, beta=None):
        ...
        self.cn_op = functools.partial(cn,
                                       crop=crop, beta=beta)
...
if __name__ == '__main__':
    ...
    # # input = input * 255
    # output = selfnorm(input)
    # # print(input.size())
    # # print(output.size())

    x=torch.randn(2,3,224,224)
    out=cn_op_2ins_space_chan(x,crop='neither', beta=1, bbx_thres=0.1, lam=None, chan=False)
    print(out.shape)
    print('ok')